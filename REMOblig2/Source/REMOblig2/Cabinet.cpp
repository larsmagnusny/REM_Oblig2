...
void UCabinet::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction * ThisTickFunction)
{
	Super::TickComponent(DeltaTime, TickType, ThisTickFunction);

	if (ItemsContained.Num() > 0)
	{
		if (!IsOpen)
		{
			for (AActor* ItemContained : ItemsContained)
			{
				if (GameMode->IsInteractble(ItemContained))
				{
					GameMode->RemoveInteractableObject(ItemContained);
				}
			}
		}
		else
		{
			for (AActor* ItemContained : ItemsContained)
			{
				if (!GameMode->IsInteractble(ItemContained))
				{
					UInteractableComponent* Component = Cast<UInteractableComponent>(ItemContained->GetComponentByClass(UInteractableComponent::StaticClass()));

					GameMode->AddInteractableObject(ItemContained, Component);
				}
			}
		}
	}
...
void UCabinet::ActivateObject(AActor * Player)
{
...
FVector UCabinet::GetActivatePosition(AActor * Player)
{
